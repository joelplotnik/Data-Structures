...
order traversal. If duplicate words are inserted, it doesnt not get inserted but a counter gets incremented. To make the cross reference table as simple as possible, 
...
and insert the word into the hash location inside the hash table. The hash function defined in lab produces only 8 collisions. When words are to be inserted into object 
